...
$_GET[$id]=encodeToIso($_GET[$id]);
	...
	$up = "update metaresposta set metaRespostaSRC='".$rin["IDM"].".jpg' where metaRespostaId='".$rin["IDM"]."'";
	$qup = mysql_query($up);
...
if($_GET["tags"]=="msg")
{
	...
	while ($rseg = mysql_fetch_assoc($qseg))
	{
		...
		$tag["temfoto"][]=$array["foto"]?$array["foto"]:"";
...
#http://www.bign.com.br/nb/az.php?op=deixartag&nottag="+segueNot+"&email="+meuEmail);
if($_GET["op"]=="deixartag")
{
	$del  = "delete from metaresposta where metarespostaDescricao like '%seguidor[t]$_GET[email][v]%' and metaRespostaDescricao like 
	'%nottag[t]$_GET[nottag][v]%' and metaId='65'";
	$tag["PODE"]="0";
	if(mysql_query($del))
	$tag["PODE"]="1";
...
if($_GET["op"]=="excluirtag")
{
	$del  = "delete from metaresposta where metarespostaDescricao like '%dono[t]$_GET[email][v]%' and metaRespostaDescricao like 
	'%nottag[t]$_GET[nottag][v]%' and metaId='64'";
	$tag["PODE"]="0";
	if(mysql_query($del))
	$tag["PODE"]="1";
...
if($_GET["tags"]=="sigo")
{
	...
	if(mysql_num_rows($q)>0) 
	{
		...
		$in = "insert into m values('','".$array["titulo"]."','".$array["nottag"]."','".$array["mensagem"]."',0,". $r["CHAVE"] .",'$_GET[email]','".$r["metaRespostaData"]."','".$array["opcoes"]."','".$array["foto"]."','".$r["dagenda"]."','".$array["subtag"]."','".$array["certa"]."')";
		$qin = mysql_query($in);
#		echo $in;
		}
	...
	$s = "select mid as IDM,mcab as NOTTAG,mdesc as TITULO,mtexto as MSG,mdata as DATA,mopcoes as OPCOES,mchave as IDNOT,mfoto as TEMFOTO
	,dagenda as DAGENDA,subtag as SUBTAG,certa as CERTA from m where mlida=0 and memail='$_GET[email]'";
	$q = mysql_query($s);
	$r = mysql_fetch_assoc($q);
	$r["DATA"]= implode("/",array_reverse(explode("-",substr($r["DATA"],0,10)))) . substr($r["DATA"],10);
	$r["DAGENDA"]= implode("/",array_reverse(explode("-",substr($r["DAGENDA"],0,10)))) . substr($r["DAGENDA"],10);
